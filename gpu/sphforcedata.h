 ...
 */
#ifndef SPHFORCEDATA_H
#define SPHFORCEDATA_H
#include "hostdata.h"
#include "basetype.h"
#include "workunit.h"
...
inline double getFlopsSPHForce(workParticle *wp,ilpTile &tile) {
...
class sphForceData : public hostData {
protected:
    bool bGravStep;
...
public:
    sphForceData() : requestBufferCount(0), resultsBufferCount(0), nTotalInteractionBlocks(0), nTotalParticles(0) {
    ...
    bool queue(workParticle *wp, ilpTile &tile, bool bGravStep) {
        ...
        auto blk = reinterpret_cast<sphForceBlk<WIDTH> *>(pHostBufIn);             // Copy the blocks to the input buffer
        ...
        wp->dFlopSingleGPU += getFlopsSPHForce(wp,tile);
        return true;
    }
    void prepare() {
        typedef sphForceBlk<WIDTH> BLK;
        ...
        auto *__restrict__ partHost = reinterpret_cast<sphForceInput *>(blkHost + nTotalInteractionBlocks);
        ...
        for ( auto &w : work ) {
            ...
            for (auto j=0; j<nP; ++j) {
                ...
                partHost[j].Omega = pInfoIn[j].Omega;
                partHost[j].vx    = pInfoIn[j].v[0];
                partHost[j].vy    = pInfoIn[j].v[1];
                partHost[j].vz    = pInfoIn[j].v[2];
        ...
        resultsBufferCount = iP * sizeof(sphForceResult);
    ...
    auto align_nP(int nP) {
        constexpr int mask = 32 * sizeof(float) / sizeof(sphForceInput) - 1;
        static_assert(32 * sizeof(float) == (mask+1)*sizeof(sphForceInput));
        return (nP + mask) & ~mask;
    }
    ...
    int inputSize(int nP=0, int nI=0) {
        ...
        return (nBlocks + nTotalInteractionBlocks) * (sizeof(BLK) + sizeof(ppWorkUnit))
               + (align_nP(nP) + nTotalParticles) * sizeof(sphForceInput)
    ...
    int outputSize(int nP=0, int nI=0) {
        ...
        return (align_nP(nP) + nTotalParticles) * sizeof(sphForceResult);
    }
};
