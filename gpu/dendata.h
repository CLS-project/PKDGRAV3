...
inline double getFlopsDen(workParticle *wp,ilpTile &tile) {
...
public:
    denData() : requestBufferCount(0), resultsBufferCount(0), nTotalInteractionBlocks(0), nTotalParticles(0) {
    ...
    bool queue(workParticle *wp, ilpTile &tile, bool bGravStep) {
        ...
        auto blk = reinterpret_cast<denBlk<WIDTH> *>(pHostBufIn);             // Copy the blocks to the input buffer
        ...
        wp->dFlopSingleGPU += getFlopsDen(wp,tile);
        return true;
    }
    void prepare() {
        typedef denBlk<WIDTH> BLK;
        ...
        auto *__restrict__ partHost = reinterpret_cast<denInput *>(blkHost + nTotalInteractionBlocks);
        ...
        resultsBufferCount = iP * sizeof(denResult);
    ...
    auto align_nP(int nP) {
        constexpr int mask = 32 * sizeof(float) / sizeof(denInput) - 1;
        static_assert(32 * sizeof(float) == (mask+1)*sizeof(denInput));
        return (nP + mask) & ~mask;
    }
    ...
    int inputSize(int nP=0, int nI=0) {
        ...
        return (nBlocks + nTotalInteractionBlocks) * (sizeof(BLK) + sizeof(ppWorkUnit))
               + (align_nP(nP) + nTotalParticles) * sizeof(denInput)
    ...
    int outputSize(int nP=0, int nI=0) {
        ...
        return (align_nP(nP) + nTotalParticles) * sizeof(denResult);
    }
};
