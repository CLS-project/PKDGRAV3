...
QSORT_TYPE *const _base = (char *)(QSORT_BASE);				\
  ...
  if (_elems > _QSORT_MAX_THRESH) {					\
    ...
    QSORT_TYPE *_hi = _lo + _size * (_elems - 1);			\
    struct {								\
    ...
    while (_QSORT_STACK_NOT_EMPTY) {					\
	  ...
	  _QSORT_SWAP (_mid, _hi, _hold, _size);			\
        if (QSORT_LT (_mid, _lo))					\
          _QSORT_SWAP (_mid, _lo, _hold, _size);			\
      } 								\
									\
      ...
      do {								\
	    ...
	    _QSORT_SWAP (_left_ptr, _right_ptr, _hold, _size);		\
          if (_mid == _left_ptr)					\
            _mid = _right_ptr;						\
  ...
  QSORT_TYPE *const _end_ptr = _base + _size*(_elems - 1);		\
    ...
    /* Find smallest element in first threshold and place it at the	\
       ...
       and the operation speeds up insertion sort's inner loop. */	\
									\
    for (_run_ptr = _tmp_ptr + _size; _run_ptr <= _thresh; _run_ptr += _size)	\
      if (QSORT_LT (_run_ptr, _tmp_ptr))				\
        _tmp_ptr = _run_ptr;						\
									\
    if (_tmp_ptr != _base)						\
      _QSORT_SWAP (_tmp_ptr, _base, _hold, _size);			\
									\
