...
//		}
		...
		mdlassert(pst->mdl,fio!=NULL);
          ...
          pkdWriteHeaderFIO(plcl->pkd, fio, in->bComove, 1./sqrt(in->dvFac), in->dTime, in->nDark, in->nGas, in->nStar);
...
int pstSetSmooth(PST pst,void *vin,int nIn,void *vout,int nOut) {
    LCL *plcl = pst->plcl;
    struct inSetSoft *in = vin;
	...
	pstSetSmooth(pst->pstLower,in,nIn,NULL,0);
	mdlGetReply(pst->mdl,rID,NULL,NULL);
	}
    else {
...
int pstPredictSmoothing(PST pst,void *vin,int nIn,void *vout,int nOut) {
    LCL *plcl = pst->plcl;
    struct inDrift *in = vin;
    ...
    if (pst->nLeaves > 1) {
       ...
       pstPredictSmoothing(pst->pstLower,in,nIn,NULL,0);
       mdlGetReply(pst->mdl,rID,NULL,NULL);
       }
    else {
