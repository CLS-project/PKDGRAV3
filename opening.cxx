...
//    k_notgrp = SIMD_I2F(SIMD_SPLATI32(k->bGroup?0:0xffffffff));
    ...
    k_Open = consts.threehalves.p*k_bMax*diCrit;
...
//	    T3 = SIMD_OR(SIMD_I2F(SIMD_CMP_GT_EPI32(iconsts.walk_min_multipole.p,blk->nc.p[i])),SIMD_CMP_LE(mink2,cOpen2));
	    T3 = (consts.walk_min_multipole.p > cvt_fvec(blk->nc.p[i])) | (mink2<=cOpen2);
	    T4 = minbnd2 > fourh2;
	    T6 = cOpen > k_Open;
	    T7 = k_notgrp;
 	    iOpenA = mask_mov(iconsts.three.pf,T2,iconsts.one.pf);
	    iOpenB = mask_mov(mask_mov(iOpenA,T4,iconsts.four.pf),T3,iOpenA);
	    P1 = mask_mov(iconsts.three.pf,T2,iconsts.two.pf);
	    P2 = mask_mov(iOpenB,T7,iconsts.zero.pf);
	    P3 = mask_mov(P2,T6,P1);
	    P4 = mask_mov(P3,T1,iconsts.eight.pf);
	    iOpen = mask_mov(iconsts.ten.pf,T0,P4);
