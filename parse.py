def parse():
    ...
    add_flag(gas,'MaxPhysicalSoft',default=0.0, dest='dMaxPhysicalSoft', type=float, help='Maximum softening in physical coordinates')
    ...
    add_flag(eeos,'EOSFloornH',default=1e-5,dest='dEOSFloornH', type=float, help='Minimum density at which the internal enrgy floor will be applied, n_H cm-3')
    add_flag(eeos,'EOSFloorMinOD',default=10,dest='dEOSFloorMinOD', type=float, help='Minimum baryonic overdensity at which the internal energy floor will be applied')
    add_flag(eeos,'EOSFloorTemp',default=1e4,dest='dEOSFloorTemp', type=float, help='Temperature at the internal energy floor, K')
    add_flag(eeos,'EOSPolyFloornH ',default=0.1,dest='dEOSPolyFloornH', type=float, help='Minimum density at which the polytropic effective EOS will be applied, nH cm-3)')
    add_flag(eeos,'EOSPolyFloorMinOD',default=10,dest='dEOSPolyFloorMinOD', type=float, help='Minimum baryonic overdensity at which the polytropic effective EOS will be applied')
    ...
    add_bool(sform,'SFThresholdDenSchaye2004',default=False,dest='bSFThresholdDenSchaye2004', help='Use the metallicity-dependent star-formation density threshold of Schaye (2004)')
    ...
    add_flag(sform,'SFindexKS',default=1.4,dest='dSFindexKS', type=float, help='Index of the Kennicutt-Schmidt star-formation law')
    add_flag(sform,'SFnormalizationKS',default=1.515e-4,dest='dSFnormalizationKS', type=float, help='Normalization of the Kennicutt-Schmidt star-formation law. Value must be consistent with the chosen IMF')
    ...
    add_bool(snfeed,'bCCSNFeedback',default=True,dest='bCCSNFeedback', help='Activate energy feedback from CCSN events')
    add_bool(snfeed,'bSNIaFeedback',default=False,dest='bSNIaFeedback', help='Activate energy feedback from SNIa events')
    add_flag(snfeed,'SNFBEfficiency',default=1.0,dest='dSNFBEfficiency', type=float, help='Efficiency of SN feedback. Asymptotic minimum efficiency if dSNFBMaxEff is provided')
    add_flag(snfeed,'SNFBMaxEff',default=0.0,dest='dSNFBMaxEff', type=float, help='Asymptotic maximum efficiency of SN feedback')
    ...
    add_flag(bh,'BHAccretionAlpha',default=1.0,dest='dBHAccretionAlpha', type=float, help='Accretion boost parameter')
    add_flag(bh,'BHAccretionCvisc',default=0.0,dest='dBHAccretionCvisc', type=float, help='Accretion viscosity parameter')
    ...
    imf = parser.add_argument_group('Stellar Initial-Mass Function (IMF)')
    add_flag(imf,'imftype', dest='achIMFType', help='Type of IMF')
    add_flag(imf,'IMFMinMass',default=0.1,dest='dIMFMinMass', help='Lower mass limit of the IMF <Mo>')
    add_flag(imf,'IMFMaxMass',default=100.0,dest='dIMFMaxMass', help='Upper mass limit of the IMF <Mo>')
    add_flag(imf,'CCSNMinMass',default=6.0,dest='dCCSNMinMass', help='Minimum mass for a star to end its life as a Core Collapse Supernova <Mo>')
    add_flag(imf,'CCSNMaxMass',default=100.0,dest='dCCSNMaxMass', help='Maximum mass for a star to end its life as a Core Collapse Supernova <Mo>')
    ...
    add_flag(stev,'SNIaExpScale',default=2e9,dest='dSNIaExpScale', help='Time scale of the exponential Delay-Time Distribution function <yr>')
    add_flag(stev,'SNIaPLScale',default=-1.1,dest='dSNIaPLScale', help='Index of the power-law Delay-Time Distribution function <dimensionless>')
    add_flag(stev,'SNIaPLInitTime',default=40e6,dest='dSNIaPLInitTime', help='Initial time for the normalization of the power-law Delay-Time Distribution function <yr>')
    add_flag(stev,'SNIaPLFinalTime',default=13.7e9,dest='dSNIaPLFinalTime', help='Final time for the normalization of the power-law Delay-Time Distribution function <yr>')
    add_flag(stev,'SNIaNumPerMass',default=2e-3,dest='dSNIaNumPerMass', help='Number of SNIa events per stellar mass <1/Mo>')
